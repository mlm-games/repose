...
fn key_frame(regions: Vec<HitRegion>) -> Frame {
    Frame {
        scene: Scene::default(),
        hit_regions: regions,
...
fn key_region(id: u64, depth: u32, parent: Option<u64>) -> HitRegion {
    HitRegion {
        id,
        depth,
        parent,
...
#[test]
fn unfocused_key_reaches_root_handler() {
    let mut rt = ReposeRuntime::new();
    let hits = Rc::new(RefCell::new(0u32));
    let h = hits.clone();
    let mut root = key_region(1, 0, None);
    root.on_key_event = Some(Rc::new(move |ev| {
        assert_eq!(ev.key, Key::Escape);
        *h.borrow_mut() += 1;
        true
    }));
    rt.cache_frame(key_frame(vec![root, key_region(2, 1, Some(1))]));
    rt.sched.focused = None;
...
#[test]
fn preview_tunnel_runs_root_first_for_every_key() {
    let mut rt = ReposeRuntime::new();
    let order = Rc::new(RefCell::new(Vec::new()));
    let root_order = order.clone();
    let mut root = key_region(1, 0, None);
    root.on_preview_key_event = Some(Rc::new(move |_| {
        root_order.borrow_mut().push("root-preview");
        false
    }));
    let child_order = order.clone();
    let mut child = key_region(2, 1, Some(1));
    child.on_preview_key_event = Some(Rc::new(move |_| {
        child_order.borrow_mut().push("child-preview");
        true
    }));
    rt.cache_frame(key_frame(vec![root, child]));
    rt.sched.focused = Some(2);

    let ev = key_down(Key::Character('a'), Modifiers::default(), false);
    assert!(rt.handle_key(&ev));
    assert_eq!(*order.borrow(), vec!["root-preview", "child-preview"]);
}

#[test]
fn bubble_reaches_root_when_focused_ignores() {
    let mut rt = ReposeRuntime::new();
    let hits = Rc::new(RefCell::new(0u32));
    let h = hits.clone();
    let mut root = key_region(1, 0, None);
    root.on_key_event = Some(Rc::new(move |_| {
        *h.borrow_mut() += 1;
        true
    }));
    let mut child = key_region(2, 1, Some(1));
    child.on_key_event = Some(Rc::new(|_| false));
    rt.cache_frame(key_frame(vec![root, child]));
    rt.sched.focused = Some(2);
...
#[test]
fn disabled_root_is_skipped() {
    let mut rt = ReposeRuntime::new();
    let mut root = key_region(1, 0, None);
    root.disabled = true;
    root.on_key_event = Some(Rc::new(|_| true));
    rt.cache_frame(key_frame(vec![root]));
    rt.sched.focused = None;
