...
impl<T> Signal<T> {
    pub fn new(value: T) -> Self {
        Self(Rc::new(RefCell::new(Inner { value, subs: Vec::new() })))
    }
    pub fn get(&self) -> T where T: Clone { self.0.borrow().value.clone() }
    pub fn set(&self, v: T) {
        let mut inner = self.0.borrow_mut();
        inner.value = v;
        let vref = &inner.value;
        for s in &inner.subs { s(vref); }
    }
...
pub fn signal<T>(t: T) -> Signal<T> { Signal::new(t) }
