...
/// GPU resources for the Slug vector glyph pipeline.
...
impl SlugPipeline {
    ...
    /// Upload active glyph data to the storage buffer.
    /// Skips the write when no cache entries have changed since the last upload.
    pub fn upload(
        &mut self,
        device: &wgpu::Device,
        queue: &wgpu::Queue,
        ...
        if needed_bytes > self.storage_buf.size() {
            ...
            *prev_generation = !0; // force write below
        }
    ...
    pub fn storage_layout(&self) -> &wgpu::BindGroupLayout {
        &self.storage_layout
    }
}
