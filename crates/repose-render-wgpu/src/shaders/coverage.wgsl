// Tinted A8 coverage composite: `color` masked by single-channel coverage.
// Used for host-rasterized coverage tiles (e.g. cached glyph tiles) that are
// uploaded once and re-composited per frame. Input and output are
// premultiplied alpha, like the text pipelines.
struct Globals {
    ndc_to_px: vec2<f32>,
    _pad: vec2<f32>,
};
...
@group(1) @binding(0) var coverage_tex: texture_2d<f32>;
@group(1) @binding(1) var coverage_samp: sampler;
...
fn fs_main(in: VSOut) -> @location(0) vec4<f32> {
    ...
    let cov = textureSample(coverage_tex, coverage_samp, atlas_uv).r;
