...
use repose_core::*;
use repose_ui::{
    Box, Column, Text, TextStyle, ViewExt, ZStack, anim::animate_f32,
...
/// Non-dismissing tooltip lifetime, mirroring Compose
/// `BasicTooltipDefaults.TooltipDuration` (1500ms).
pub const TOOLTIP_AUTO_DISMISS_MS: u64 = 1500;

/// State controlling tooltip visibility.
///
/// Non-persistent tooltips auto-dismiss after [`TOOLTIP_AUTO_DISMISS_MS`];
/// persistent ones stay until dismissed (for actionable content). Mirrors
/// Compose `TooltipState.isPersistent`.
    ...
    pub fn show(&self) {
        *self.timer.borrow_mut() = None;
        ...
        if !self.persistent {
            self.timer
                .borrow_mut()
                .replace(timer::delay(Duration::from_millis(TOOLTIP_AUTO_DISMISS_MS), {
...
///
/// The popup renders in `overlay` (never clipped by parents or scroll
/// containers) and is flipped/clamped into the window container like Compose
/// `TooltipBox` + `TooltipPositionProviderImpl`.
///
